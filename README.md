...
> *你一定有过这样的时刻——*  
> *某个深夜读过一篇文章，某次会议前翻到一段资料，*  
> *它们像流星划过浏览器，明明看见过，却再也找不回来。*  
>
> *Deja Browse，取自法语 Déjà Vu——"似曾相识"。*  
> *我们给它起了一个中文名：**拾迹**。*  
> *拾起你浏览过的每一道痕迹，让记忆不再只是记忆，而是随时可以重逢的旧友。*  
>
> *你负责漫游，拾迹负责记住。*
...
- **[hamlet0x62](https://github.com/hamlet0x62)** — 项目的最初构想者，提出了"用语义搜索找回浏览记忆"这个金点子，为拾迹定下了方向
- **[chapensis](https://github.com/chapensis)** — 作为第一个种子用户，在产品打磨期提供了大量真实场景反馈和宝贵改进建议
...
> **更新插件前请先备份数据**  
> 如果你是“移除后重装”或更换 `dist/` 重新加载，建议先在 Options 页面执行一次**导出 `.sqlite3` 或 JSON**，更新完成后再**导入数据**恢复。  
> 仅点击 `chrome://extensions` 的“重新加载（Reload）”通常不会清空数据，但仍建议在重要升级前先导出备份。
